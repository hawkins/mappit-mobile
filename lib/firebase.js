...
async function setTopology(name, description) {
  ...
  return db.ref(`/topologys/${name}`).set(params);
}

// Returns a list of all Topologys
async function getTopologys() {
  const snapshot = await db.ref(`/topologys`).once("value");
...
async function getMyTopologys() {
  ...
  const myTopologys = topologys.filter(t => {
    if (!t.subscribers) return false;
    return Object.keys(t.subscribers).indexOf(user_id) > -1;
  });

  // Return just the keys
  return myTopologys.map(t => t.key);
}

// Get the data of a topology given its key
async function getTopology(key) {
  const snapshot = await db.ref(`/topologys/${key}`).once("value");
...
async function getPosts() {
  const snapshot = await db.ref(`/topologys/${name}/posts`).once("Value");
...
async function getPost(name, id) {
  const snapshot = await db.ref(`/topologys/${name}/posts/${id}`).once("Value");
...
async function setPost(name, title, content, lat, lon, type) {
  ...
  return db.ref(`/topologys/${name}/posts`).push(params);
}
