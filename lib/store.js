...
export default class Store {
  ...
  async loadSubTopologys() {
    ...
    let homeTopology = null;
    for(i = 0; i < subTops.length; i++) {
      let tempPost = null;
      homeTopology = await firebase.getTopology(subTops[i]);
      if (homeTopology) {
        if (homeTopology.posts != null) {
          homeTopology.posts = Object.keys(homeTopology.posts).map(
            key => homeTopology.posts[key]
          );
        } else {
          homeTopology.posts = [];
        }
        if (this.user && homeTopology.subscribers[this.user.uid])
        this.subscribed = homeTopology.subscribers[this.user.uid] !== "false";
      }
    }
    console.log(homeTopology);
    // Clean up Home Topology
    homeTopology['key'] = "Home";
    homeTopology['date'] = null;
    homeTopology['description'] = null;
    homeTopology['ownwer'] = null;
